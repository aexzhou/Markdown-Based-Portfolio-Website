...
class NodeMarkdownBlog {
    ...
    async getPosts() {
        const postsData = await getMarkdownData("views/projects")
    ...
    // Method to only return posts data for spotlight posts
    async getSpotlightPosts() {
        const postsData = await getMarkdownData("views/projects")
        const SpotlightPostsData = postsData
            .filter(obj => obj[1].frontmatter.spotlight === "true")
            .map((obj) => {
                return { ...obj, date: new Date(obj[1].frontmatter.date) }
            })
            .sort((objA, objB) => Number(objB.date) - Number(objA.date))
        return SpotlightPostsData
    }

}

export const { parseFrontMatter, getFiles, getMarkdownData, getPages, getPosts, getSpotlightPosts} = new NodeMarkdownBlog()